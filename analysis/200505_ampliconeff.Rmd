...
amplicon=amplicon %>%
  mutate(pool.y=ifelse(pool=="nCoV-2019_1", 0,1))
...
ggplot(amplicon, aes(x=length, fill=pool))+geom_histogram()+facet_wrap(~pool)

```

Then I want to plot the location of the amplicons along the genome

first pull in the genome (downloaded from )
...
ggplot(amplicon)+geom_segment(aes(x=amp.start, xend=amp.end, y=pool.y, yend=pool.y, color=pool))+ylim(0,10)
